...
void Game::checkAction(Joystick* player) {
  ...
  if (player[playerTurn].buttState == 0 && board[cursorX][cursorY] == -1) {
    board[cursorX][cursorY] = playerTurn;
...
int Game::checkWin() {
  for (int player = 0; player < 2; player++) {
    ...
    for (int column = 0; column < 3; column++) {
      if (board[column][0] == 0  &&  board[column][1] == player && board[column][2] == player) {
        return 0;
      }
    }
